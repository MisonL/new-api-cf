...
import { forwardSpeechCreate, forwardTranscriptionCreate, forwardTranslationCreate } from '../lib/upstream';
import { parseTranscriptionRequest, parseTranslationRequest, speechCreateRequestSchema } from '../schemas/audio';
...
export function createAudioRouter() {
  ...
  router.post('/v1/audio/translations', async (c) => {
    const formData = await c.req.formData().catch(() => {
      throw new ApiError(400, 'INVALID_FORM_DATA', 'request body must be valid form data');
    });
    const request = parseTranslationRequest(formData);
    const config = getRuntimeConfig(c.env);
    const access = await requireRelayAccess(c, config);
    await enforceRelayRateLimit(c.env, access, config.relayRateLimitPerMinute);
    return forwardTranslationCreate(c.env, request.model, formData, config, access);
  });
