...
import { forwardImageEdit, forwardImageGeneration, forwardImageVariation } from '../lib/upstream';
import { imageGenerationRequestSchema, parseImageEditRequest, parseImageVariationRequest } from '../schemas/images';
...
export function createImagesRouter() {
  ...
  router.post('/v1/images/variations', async (c) => {
    const formData = await c.req.formData().catch(() => {
      throw new ApiError(400, 'INVALID_FORM_DATA', 'request body must be valid form data');
    });
    const request = parseImageVariationRequest(formData);
    const config = getRuntimeConfig(c.env);
    const access = await requireRelayAccess(c, config);
    await enforceRelayRateLimit(c.env, access, config.relayRateLimitPerMinute);
    return forwardImageVariation(c.env, request.model, formData, config, access);
  });
