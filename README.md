...
- 若未配置上游环境变量，`/api/models`、`/v1/chat/completions`、`/v1/embeddings` 和 `/v1/responses` 会显式失败
...
- 若绑定 `RELAY_LIMITER` Durable Object 且配置 `RELAY_RATE_LIMIT_PER_MINUTE`：
  - `/v1/chat/completions`、`/v1/embeddings` 与 `/v1/responses` 会按调用方执行每分钟速率门禁
...
- `/v1/models`、`/v1/chat/completions`、`/v1/embeddings` 与 `/v1/responses` 当前要求：
  - admin session
  - 或 D1 API token
